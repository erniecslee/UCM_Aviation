...
function step(dt){
  ...
  if(act){
    ...
    if(ag&&ag.hdgCmd!=null){ lim=30; hdgCmd=ag.hdgCmd; }
      ...
      else{ lim=30; s.guide=g; hdgCmd=trkToHdg(wrap360(g.course-clamp(g.xtkR*15,-45,45)));
    ...
    if(hdgCmd!=null){ let err=wrap180(hdgCmd-s.hdg);
      if(s.hdgDir && s.modes.roll==='HDG SEL' && Math.abs(err)>1){ const de=s.hdgDir>0?wrap360(hdgCmd-s.hdg):-wrap360(s.hdg-hdgCmd); if(Math.abs(de)>180) err=de; else s.hdgDir=0; }
  ...
  let vc=s.vs; const spdHold=()=>{ const ee=s.ias-s.tsRef; s.pI=clamp(s.pI+25*ee*dt,-5000,4500); return clamp(s.pI+350*ee,-5000,4500); };
