...
function spdRestr(){ try{ const c=constraint(e.perf.spdRest||'250/10000'); const s=+c.speed, a=+altitude(c.altitude); return {spd:Number.isFinite(s)?s:250,alt:Number.isFinite(a)?a:10000}; }catch(err){ return {spd:250,alt:10000}; } }
...
function fmcTargetIas(phase,alt,ahead){
  ...
  const rs=spdRestr(); if(alt<rs.alt+ (phase==='DES'?300:0)) ias=Math.min(ias,rs.spd);
...
function vnavGuide(dt){
  ...
  const spdHold=(ts)=>{ const ee=s.ias-(s.tsRef==null?ts:s.tsRef); s.pI=clamp(s.pI+25*ee*dt,-5000,4500); return clamp(s.pI+350*ee,-5000,4500); };
  ...
  if(phase==='CLB'){
    ...
    for(const it of ahead){ if(it.disco||!it.alt) continue; const c=it.alt; const cap=(c.type==='B'||c.type==='@')?c.alt:(c.type==='W'?c.alt2:null); if(cap!=null&&cap>=ai-50&&cap<tgt){ tgt=cap; why='CON'; } }
    ...
    const pathAlt=path?pathAltAt(path,dtgNow):ai, tgt=Math.max(pathAlt,mcpTop), err=ai-tgt;
...
function vnavPress(){
  ...
  S.vn={intv:false}; S.pI=S.vs; const ai=altInd(), crz=crzAltFt();
