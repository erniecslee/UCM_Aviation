...
function drawPFD(){
  ...
  if(s.fdShown&&!(s.flareEngaged&&s.landVariant!=='FO')){ const ctr=s.flareEngaged&&s.landVariant==='FO'; const fy=ctr?cy:clamp(cy-s.fdPitch*pk,AY+40,AY+AH-40), fx=ctr?cx:cx+clamp(s.fdRoll*5,-80,80); L(x,cx-100,fy,cx+100,fy,C.mag,5); L(x,fx,cy-78,fx,cy+78,C.mag,5); }
  ...
  if(cmd){ if(S.landStat==='NO AUTOLAND') T(x,'NO AUTOLAND',cx,116,C.amb,22,'center',700); else T(x,S.landStat||'CMD',cx,116,C.grn,28,'center',700); } else if(fdAny) T(x,'FD',cx,116,C.grn,28,'center',700);
  ...
  if(S.app.locCap&&m.ap&&!m.ap2) T(x,'SINGLE CH',cx,134,C.amb,13,'center',700);
  if(chg('pitch')&&false){}
  /* speed tape */
  ...
  x.restore();
  if(!s.gnd){ barber(x,sx+sw+1,st,9,Math.max(0,Math.min(sh,ys(vmax)-st))); { const y0=clamp(ys(vmin),st,st+sh); barber(x,sx+sw+1,y0,9,st+sh-y0); } }
  x.strokeStyle=C.amb; x.lineWidth=2; { const y1=clamp(ys(vmin+18),st,st+sh), y0=clamp(ys(vmin),st,st+sh); L(x,sx+sw+13,y1,sx+sw+13,y0,C.amb,2); L(x,sx+sw+8,y1,sx+sw+13,y1,C.amb,2); }
  ...
  if(V40){ const list=[['UP',0],['1',1],['5',5],['15',15],['25',25]].filter(([n,d])=>ldg?true:d<=Math.min(tof,s.cfg.flapSel)).map(([n,d])=>[n,maneuverSpeed(d)]);
  ...
  if((s.gnd&&!s.tdT)||(s.to&&s.to.active)){ const P=(typeof e!=='undefined'&&e&&e.perf)||{}, ok=/^\d{2,3}$/.test(P.v1||'')&&/^\d{2,3}$/.test(P.vr||'')&&/^\d{2,3}$/.test(P.v2||'')&&!P.vSpeedsOff&&!s.tdT;
    ...
    else if(s.gnd){ x.fillStyle='#000'; x.fillRect(sx+4,st+6,sw-8,22); x.strokeStyle=C.amb; x.lineWidth=2; x.strokeRect(sx+4,st+6,sw-8,22); T(x,'NO V SPD',sx+sw/2,st+17,C.amb,13,'center',700); } }
  const by=clamp(ys(ts),st+8,st+sh-8); x.strokeStyle=C.mag; x.lineWidth=3; x.beginPath(); x.moveTo(sx+sw+2,by); x.lineTo(sx+sw+16,by-9); x.lineTo(sx+sw+16,by+9); x.closePath(); x.stroke();
  ...
  if(s.ias>=30) T(x,String(Math.round(s.ias)),sx+64,mid,(!s.gnd&&s.ias<vmin)?C.red:'#fff',34,'right',700,1);
  ...
  if(!s.gnd && s.ias<vmin+3 && Math.floor(s.t*3)%2===0) T(x,'SPEED LOW',cx,AY+46,C.red,15,'center',700);
  /* altitude tape */
  ...
  const ba=clamp(ya(m.alt),st+10,st+sh-10); x.strokeStyle=C.mag; x.lineWidth=3; x.beginPath(); x.moveTo(ax+2,ba-10); x.lineTo(ax+24,ba-10); x.lineTo(ax+24,ba+10); x.lineTo(ax+2,ba+10); x.closePath(); x.stroke();
  x.restore();
  ...
  T(x,ath>0?String(ath):'',ax+32,mid,'#fff',34,'left',700,1); T(x,ares,ax+32+(ath>9?42:24),mid+2,'#fff',24,'left',700,1);
  ...
  x.fillStyle='#625f5f'; x.beginPath(); x.moveTo(vx,mid-165); x.lineTo(vx+vw*0.39,mid-165); x.lineTo(vx+vw,mid-88); x.lineTo(vx+vw,mid+88); x.lineTo(vx+vw*0.39,mid+165); x.lineTo(vx,mid+165); x.closePath(); x.fill();
  ...
  if(Math.abs(hb)<=100){ const an=hb*D2R; x.setLineDash([9,7]); L(x,hx,hy,hx+hr*Math.sin(an),hy-hr*Math.cos(an),C.mag,2); x.setLineDash([]); x.save(); x.translate(hx+hr*Math.sin(an),hy-hr*Math.cos(an)); x.rotate(an); x.strokeStyle=C.mag; x.lineWidth=3; x.beginPath(); x.moveTo(-9,10); x.lineTo(9,10); x.lineTo(-9,-10); x.lineTo(9,-10); x.closePath(); x.stroke(); x.restore(); }
  x.restore();
...
function drawND(){
  ...
  if(EF.sw.arpt&&typeof e!=='undefined'&&e){ for(const id of [e.route.origin,e.route.destination]){ const a=db.airports[id]; if(!a) continue; const p=w2s(a); x.strokeStyle=C.cyn; x.lineWidth=2; x.beginPath(); x.arc(p[0],p[1],7,0,7); x.stroke(); T(x,id,p[0]+12,p[1]-3,C.cyn,14,'left',700); } }
  ...
  const bb=(exp?clamp(hb,-66,66):hb)*D2R; if(!pln){ x.save(); x.translate(cx+R*Math.sin(bb),cy-R*Math.cos(bb)); x.rotate(bb); x.strokeStyle=C.mag; x.lineWidth=3; x.beginPath(); x.moveTo(-11,9); x.lineTo(11,9); x.lineTo(-11,-9); x.lineTo(11,-9); x.closePath(); x.stroke(); x.restore();
