...
function initFromFMC(phase){
  ...
  S.modes.pitch='ALT HOLD'; S.modes.at='MCP SPD';
...
function initAtRunway(){
  ...
  const R=best.inf; e.active.runway=R.id;
...
function initPreflight(){
  newState(); const A=db.airports[e.scenario.origin]; const rw=A.runways.map(r=>runwayInfo(A.id,r.id)).filter(Boolean).sort((a,b)=>b.lenFt-a.lenFt)[0];
...
async function loadSituation(orig,dest,phase){
  ...
  try{
    ...
    e.newFlight(o,d,phase==='runway'?'takeoff':phase); sessionStarted=true; page=phase==='runway'?'TAKEOFF REF':(phase==='preflight'?'IDENT':'LEGS'); pageNo=0; clear();
...
{ const ex0=Engine.prototype.execute; Engine.prototype.execute=function(){ const r=ex0.apply(this,arguments); try{ S.legKey=''; lnavGuide(); }catch(err){} return r; }; }
...
FMCUI.todEstimate=todEstimate; FMCUI.inHold=idx=>!!(S.hold&&S.hold.idx===idx); FMCUI.actualN1=()=>S.n1; FMCUI.altNow=()=>S.alt;
let cduT=0;
...
requestAnimationFrame(frame);
loadSituation('KMCI','KSTL','cruise');
