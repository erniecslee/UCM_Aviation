...
/* vector (heading / course) legs: the FMC flies the leg's heading until the leg terminates (VI/CI: intercept the next leg, VA/CA/FA: altitude, VM/FM/VD: until the next fix is ahead) */
const VECT=['VI','VA','VM','VD','CA','CI','CD','CR','VR','FM','FA','FD'];
const isVect=l=>!!l&&VECT.includes(l.path)&&!l.disco;
const flyableLeg=l=>isVect(l)||(isPos(l)&&LNAV_PATHS.includes(l.path));
function legCourseM(l){ const c=String((l&&l.course)||''); return /^\d+$/.test(c)?(c.length>=4?+c/10:+c):null; }
...
function lnavGuide(){
  ...
  if(isVect(l)) return vectorGuide(r,l);
  ...
  if(S.legKey!==key){ S.legKey=key; S.legStart=prev?{lat:prev.lat,lon:prev.lon}:{lat:S.lat,lon:S.lon}; S.arcStart=S.legStart; S.arcDone=false;
    ...
    if(!prev&&l.path==='CF'&&isPos(l)&&legCourseM(l)!=null){ const cT=toTrue(legCourseM(l),S.magvar); S.legStart=destPoint(l,wrap360(cT+180),Math.max(distance(S,l)*1.3,3)); S.arcStart=S.legStart; } }   /* course to a fix with no previous fix: the line is the inbound course through the fix */
...
function vectorGuide(r,l){
  const cm=legCourseM(l), hdgT=cm==null?S.hdg:toTrue(cm,S.magvar), nx=r.legs[r.index+1], key='V|'+r.index+'|'+l.path+'|'+l.id;
  if(S.legKey!==key){ S.legKey=key; S.legStart={lat:S.lat,lon:S.lon}; S.legArc=null; S.arcDone=true; }
  let done=false;
  if(['VA','CA','FA'].includes(l.path)){ const a=parseInt(l.alt1,10); if(Number.isFinite(a)&&altInd()>=a-20) done=true; }
  else if(nx&&!nx.disco){
    const nc=legCourseM(nx);
    if((l.path==='VI'||l.path==='CI')&&nx.path==='CF'&&nc!=null&&isPos(nx)){ const cT=toTrue(nc,S.magvar), p=llXY(S,nx), xtk=p.x*Math.cos(cT*D2R)-p.y*Math.sin(cT*D2R), ang=Math.abs(wrap180(S.hdg-cT)), R=S.tas*S.tas/(68626*Math.tan(25*D2R)), lead=Math.max(0.5,R*Math.tan(Math.min(ang,150)*D2R/2));
      if(Math.abs(xtk)<=lead&&xtk*Math.sin((S.hdg-cT)*D2R)<=0) done=true; }
    else if(isPos(nx)&&Math.abs(wrap180(bearing(S,nx)-S.hdg))<=45&&distance(S,nx)>0.5) done=true;
  }
  if(done){ sequenceLeg(); S.legKey=''; return lnavGuide(); }
  return {leg:l,r,course:hdgT,heading:true,along:0,L:1e3,xtkR:0,lead:0.6,toGo:999,dist:isPos(l)?distance(S,l):0,vector:true};
}
