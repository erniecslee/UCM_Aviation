...
function standPress(a){
  const c=S.cfg;
  if(a.startsWith('fl')){ const d=CFG.flapDetents[+a.slice(2)]; if(d<c.flapSel&&!S.gnd) S.flapRetracted=true; if(d!==c.flapSel){ c.flapSel=d; if(S.ias>(CFG.flapVFE[d]||340)) msg('Flap placard speed exceeded: '+d+' is limited to '+CFG.flapVFE[d]+' kt.'); } return true; }
